"""
This module tests a number of possibilities for filling a polygon. we here use the QGraphicsItems from the QT library
as they are used in pyqtgraph and so very valid for X-ray suite:
---------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------
Tested possibilits:

1 - Using the QGraphicsItem contains function (test_qt_graphics_object)
2 - Using a distance measure to the center of a circle -- works only for circles or ellipses (with modified equation)
...
for n in xrange(num):
    ...
    mask_data = np.zeros((size, size))
    t1 = time.time()
    my_object = QtGui.QGraphicsRectItem(size/2, size/2, size/4., size/4,)
