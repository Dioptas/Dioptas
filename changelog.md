...
- **Dropped frames no longer scramble a map, silently.** A scan that lost one image previously could not fill its grid at all — nine points do not factor into 3×3 — and every point after the gap sat one cell too early, in a map that still looked plausible. The cell list now has one row per grid cell rather than per file, so a gap is visible: rows can be dragged or nudged one place at a time to rearrange the map, and right-clicking one inserts a blank for a missing frame, removes a blank, or leaves a bad point (a saturated frame, a beam dump) out without moving the others. **Check filename numbering** in the new Grid dialog finds numbers missing from the file names and inserts a blank for each, repairing a dropped frame in one click. All of these are icon buttons beside the list as well as right-click actions, and fade out when they do not apply.

- **The map mode's right-hand side is now three tabs — Image, Points, Layers.** The detector image previously sat beside the point list, and a narrow window squeezed the two into each other until neither was usable; as a tab it gets the full panel, and so do the tables. The image only shows which point is selected, so it does not need to be always on screen.
