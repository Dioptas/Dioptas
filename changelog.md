...
- **Calibration is now a step-by-step wizard.** Instead of one panel with every control at once, the workflow walks through three pages showing only what the current step needs: load and orient the image and describe the detector; pick peaks on the rings; choose calibrant, wavelength and distance and calibrate. A step indicator shows where you are and what is done, Next only unlocks when the step's prerequisites exist (an image, picked peaks), and a hint above the image always names the next action, including a live count of picked peaks and rings. Calibrate and Refine stay greyed out until they can actually run, with the tooltip explaining what is missing — replacing the error dialogs that previously appeared only after the click. The pyFAI and Fit2d parameter pages are unchanged for experts, and loading an existing calibration works as before.
