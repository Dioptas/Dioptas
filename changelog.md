...
- The validation step shows image, cake and pattern side by side instead of tabs. Clicking any of the three places a green marker at the same 2θ in all of them — the iso-2θ ring on the image, a vertical line in the cake, the position line in the pattern — for checking that rings, cake lines and peaks line up. Loaded phases are overlaid in every view with semi-transparent lines (rings on the image), so their match with the measured peaks is visible at a glance. Parameters can be fixed to chosen values before calibrating: the Advanced box on the Calibrate step carries fit checkboxes and value fields for the rotations and PONI, synced with the fitted values afterwards.
