...
- **Calibration is now a step-by-step wizard.** A stepper across the top — 1. Image, 2. Pick Rings, 3. Calibrate — shows where you are and what is done, and the panel shows only what the current step needs: load and orient the image and describe the detector; pick peaks on the rings; choose calibrant, wavelength and distance and calibrate. Next unlocks when a step's prerequisites exist (an image, picked peaks), and completed steps can be revisited by clicking the stepper. Actions appear where and when they can actually run: Calibrate sits at the end of step 3 once peaks are picked, and Refine, the fitted pyFAI/Fit2d parameters and Save Calibration appear only after a calibration exists — replacing the always-present buttons and the error dialogs that previously fired after the click. Loading an existing calibration still works from anywhere and jumps straight to the result page.
