...
class HorHistogramLUTItem(GraphicsWidget):
    ...
    def imageChanged(self, autoLevel=False, autoRange=False):
        ...
        h[1,:] = np.log(h[1,:])
        self.plot.setData(*h)
        self.vb.setRange(yRange = [0, 1.1*np.max(h[1,:])], xRange = [0, np.max(h[0,:])])


        if self.percentageLevel:
            hist_x_range = np.max(h[0,:]) - np.min(h[0,:])
            if self.first_image:
                self.region.setRegion([h[0,0],h[0,-1]])
                self.old_hist_x_range = hist_x_range
                self.first_image = False
            else:
                region_fraction = np.array(self.region.getRegion())/self.old_hist_x_range
                self.region.setRegion(region_fraction*hist_x_range)
                self.old_hist_x_range = hist_x_range
        else:
            if self.region.getRegion()[1]>h[0,-1]:
                self.region.setRegion([self.region.getRegion()[0], h[0,-1]])
