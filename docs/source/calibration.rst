...
In case the your calibrant is not available please add you own calibrant in the dioptas/calibrants folder as a text file
containing a list of d-spacings, Dioptas will automatically have this calibrant available as in the combobox after a
restart.
Different detector orientations can be accommodated by rotation or flipping of the image.
...
If the automatic peak searching fails (when Dioptas fails to select other peaks in addition to the first diffraction
ring) there are several options:
...
This will calculate the geometric parameters based on the current peak selection and then do automatically refine the
...
user interface.
To fast reuse the current calibration later, the calibration can be reloaded by clicking **Load Calibration**.
