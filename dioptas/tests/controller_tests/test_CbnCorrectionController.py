...
class CbnCorrectionControllerTest(QtTest):
    def setUp(self):
    ...
    def _enable_cbn_correction(self):
        self.correction_widget.cbn_seat_gb.setChecked(True)
        self.correction_controller.cbn_groupbox_changed()

    def test_enable_cbn_correction(self):
        self._load_and_calibrate()
        self._enable_cbn_correction()
        self.assertTrue(self.model.img_model.has_corrections())
        self.assertIsNotNone(
            self.model.img_model.img_corrections.get_correction("cbn")
        )

    def test_disable_cbn_correction(self):
        self._load_and_calibrate()
        self._enable_cbn_correction()
        self.assertTrue(self.model.img_model.has_corrections())

        self.correction_widget.cbn_seat_gb.setChecked(False)
        self.correction_controller.cbn_groupbox_changed()
        self.assertFalse(self.model.img_model.has_corrections())

    def test_cbn_without_calibration_shows_error(self):
        ...
        self.assertFalse(self.correction_widget.cbn_seat_gb.isChecked())
        QtWidgets.QMessageBox.critical.assert_called_once()

    def test_cbn_correction_affects_image_data(self):
        self._load_and_calibrate()
    ...
    def test_parameter_change_updates_correction(self):
        ...
        correction_before = (
            self.model.img_model.img_corrections.get_correction("cbn").get_data().copy()
        )
    ...
    def test_plot_btn_shows_correction(self):
        ...
        click_button(self.correction_widget.cbn_seat_plot_btn)
        self.assertEqual(self.correction_widget.cbn_seat_plot_btn.text(), "Back")

        click_button(self.correction_widget.cbn_seat_plot_btn)
        self.assertEqual(self.correction_widget.cbn_seat_plot_btn.text(), "Plot")

    def test_corrections_removed_resets_cbn(self):
        QtWidgets.QMessageBox.critical = MagicMock()
        self._load_and_calibrate()
        self._enable_cbn_correction()
        self.assertTrue(self.correction_widget.cbn_seat_gb.isChecked())

        self.correction_controller.corrections_removed()

        self.assertFalse(self.correction_widget.cbn_seat_gb.isChecked())

    def test_update_gui_restores_cbn_state(self):
        self._load_and_calibrate()
        self._enable_cbn_correction()
    ...
    def test_cbn_plot_resets_other_plot_btns(self):
        self._load_and_calibrate()
        self._enable_cbn_correction()
