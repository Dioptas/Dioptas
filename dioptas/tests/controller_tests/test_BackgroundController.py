...
def test_configuration_selected_changes_auto_background_widgets(background_controller, pattern_controller,
                                                                integration_widget, dioptas_model, qtbot):
    widget = integration_widget
    model = dioptas_model

    model.pattern_model.load_pattern(os.path.join(unittest_data_path, 'pattern_001.chi'))
    click_button(widget.qa_bkg_pattern_btn)
    assert widget.bkg_pattern_gb.isChecked()

    model.add_configuration()
    assert not model.pattern_model.pattern.auto_background_subtraction
...
def test_changing_unit(background_controller, pattern_controller, integration_widget, dioptas_model, qtbot):
    widget = integration_widget
    model = dioptas_model

    model.calibration_model.load(os.path.join(unittest_data_path, 'LaB6_40keV_MarCCD.poni'))
    model.img_model.load(os.path.join(unittest_data_path, "image_001.tif"))
...
def test_save_fit_background_pattern(background_controller, pattern_controller, integration_widget, dioptas_model,
                                     qtbot, tmpdir):
    widget = integration_widget
    model = dioptas_model

    QtWidgets.QFileDialog.getSaveFileName = MagicMock(return_value=os.path.join(tmpdir, "test_bg.xy"))
