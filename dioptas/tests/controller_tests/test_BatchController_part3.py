...
def test_set_unit(batch_controller, batch_widget, integration_widget, dioptas_model, load_proc_data):
    ...
    batch_widget.stack_plot_widget.img_view.img_view_rect = MagicMock(return_value=DummyViewRect())
...
def test_show_phases(batch_controller, phase_controller, batch_widget, dioptas_model, load_proc_data):
    # Load phases
...
def test_plot_image(batch_controller, batch_widget, dioptas_model, load_proc_data):
    ...
    assert batch_widget.position_widget.mouse_pos_widget.clicked_pos_widget.x_pos_lbl.text() == f'Img: 15'


def test_plot_pattern(batch_controller, dioptas_model, load_proc_data):
    batch_controller.plot_pattern(10, 15)

    assert dioptas_model.pattern_model.pattern.data[0][0] == pytest.approx(9.6926780, 0.001)
...
def test_update_y_axis(batch_controller, batch_widget, load_proc_data):
    ...
    assert batch_widget.stack_plot_widget.img_view.left_axis_cake.range[0] == pytest.approx(2.904, 0.01)
    assert batch_widget.stack_plot_widget.img_view.left_axis_cake.range[1] == pytest.approx(30.321324, 0.01)


def test_click_in_2d_widget_sends_clicked_change(batch_controller, batch_widget, dioptas_model, load_proc_data):
...
def test_clicked_tth_change_signal_changes_line_pos(batch_controller, batch_widget, dioptas_model, load_proc_data):
    batch_controller.plot_batch()

    # in plot range
    dioptas_model.clicked_tth_changed.emit(12)
    new_line_pos = batch_widget.stack_plot_widget.img_view.vertical_line.pos()[0]
    assert get_partial_value(dioptas_model.batch_model.binning, new_line_pos) == pytest.approx(12, 0.01)

    #  lower than range
    dioptas_model.clicked_tth_changed.emit(8)
    assert batch_widget.stack_plot_widget.img_view.vertical_line not in \
           batch_widget.stack_plot_widget.img_view.img_view_box.addedItems

    # in range again
    dioptas_model.clicked_tth_changed.emit(12)
    assert batch_widget.stack_plot_widget.img_view.vertical_line in \
           batch_widget.stack_plot_widget.img_view.img_view_box.addedItems
    new_line_pos = batch_widget.stack_plot_widget.img_view.vertical_line.pos()[0]
    assert get_partial_value(dioptas_model.batch_model.binning, new_line_pos) == pytest.approx(12, 0.01)
...
def test_integrate(batch_controller, batch_widget, integration_widget, dioptas_model, load_proc_data):
    batch_widget.position_widget.step_series_widget.start_txt.blockSignals(True)
    batch_widget.position_widget.step_series_widget.stop_txt.blockSignals(True)
    batch_widget.position_widget.step_series_widget.start_txt.setValue(5)
    batch_widget.position_widget.step_series_widget.stop_txt.setValue(28)
    ...
    dioptas_model.calibration_model.load(os.path.join(unittest_data_path, 'lambda', 'L2.poni'))
