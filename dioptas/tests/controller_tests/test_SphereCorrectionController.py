...
class SphereCorrectionControllerTest(QtTest):
    def setUp(self):
    ...
    def test_enable_sphere_correction(self):
        self._load_and_calibrate()
        self._enable_sphere_correction()
        self.assertTrue(self.model.img_model.has_corrections())
        self.assertIsNotNone(
            self.model.img_model.img_corrections.get_correction("sphere")
        )

    def test_disable_sphere_correction(self):
        self._load_and_calibrate()
        self._enable_sphere_correction()
        self.assertTrue(self.model.img_model.has_corrections())

        self.correction_widget.sphere_gb.setChecked(False)
        self.correction_controller.sphere_groupbox_changed()
        self.assertFalse(self.model.img_model.has_corrections())

    def test_sphere_without_calibration_shows_error(self):
        ...
        self.assertFalse(self.correction_widget.sphere_gb.isChecked())
    ...
    def test_invalid_formula_shows_error(self):
        ...
        self._enable_sphere_correction(formula="InvalidXYZ123")
    ...
    def test_mu_label_updated_on_enable(self):
        self._load_and_calibrate()
        self._enable_sphere_correction()
        mu_text = self.correction_widget.sphere_mu_lbl.text()
    ...
    def test_mu_label_reset_on_disable(self):
        ...
        self.correction_widget.sphere_gb.setChecked(False)
        self.correction_controller.sphere_groupbox_changed()
        self.assertEqual(self.correction_widget.sphere_mu_lbl.text(), "μ:")

    def test_sphere_correction_affects_image_data(self):
        self._load_and_calibrate()
    ...
    def test_parameter_change_updates_correction(self):
        ...
        correction_before = (
            self.model.img_model.img_corrections.get_correction("sphere").get_data().copy()
        )
        ...
        correction_after = (
            self.model.img_model.img_corrections.get_correction("sphere").get_data()
        )
    ...
    def test_plot_btn_shows_correction(self):
        ...
        click_button(self.correction_widget.sphere_plot_btn)
        self.assertEqual(self.correction_widget.sphere_plot_btn.text(), "Back")

        click_button(self.correction_widget.sphere_plot_btn)
        self.assertEqual(self.correction_widget.sphere_plot_btn.text(), "Plot")

    def test_corrections_removed_resets_sphere(self):
        QtWidgets.QMessageBox.critical = MagicMock()
        self._load_and_calibrate()
        self._enable_sphere_correction()
        self.assertTrue(self.correction_widget.sphere_gb.isChecked())
    ...
    def test_update_gui_restores_sphere_state(self):
        self._load_and_calibrate()
        self._enable_sphere_correction()
        ...
        self.assertTrue(self.correction_widget.sphere_gb.isChecked())
        self.assertIn("1/mm", self.correction_widget.sphere_mu_lbl.text())

    def test_sphere_plot_resets_other_plot_btns(self):
        self._load_and_calibrate()
        self._enable_sphere_correction()
    ...
    def test_empty_formula_does_not_enable(self):
        self._load_and_calibrate()
        self._enable_sphere_correction(formula="")
        self.assertFalse(self.correction_widget.sphere_gb.isChecked())
        self.assertFalse(self.model.img_model.has_corrections())
