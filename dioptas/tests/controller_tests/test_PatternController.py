...
class PatternControllerTest(QtTest):
    ...
    def test_configuration_selected_changes_active_unit_btn(self):
        ...
        self.assertEqual(
            ...
            "<span style='color: #ffffff'>2θ (°)</span>",
        )
        ...
        self.assertEqual(
            self.widget.pattern_widget.pattern_plot.getAxis("bottom").labelString(),
            "<span style='color: #ffffff'>Q (Å⁻¹)</span>",
        )
    ...
    def test_save_pattern_without_background(self):
        QtWidgets.QFileDialog.getSaveFileName = MagicMock(
            return_value=os.path.join(unittest_data_path, "test.xy")
        )
    ...
    def test_click_pattern_sends_tth_changed_signal(self):
        ...
        self.widget.integration_pattern_widget.pattern_view.mouse_left_clicked.emit(
            10, 300
        )
    ...
    def test_click_pattern_changes_position_line(self):
        ...
        prior_pos = (
            self.widget.integration_pattern_widget.pattern_view.pos_line.getPos()
        )
        self.widget.integration_pattern_widget.pattern_view.mouse_left_clicked.emit(
            12, 300
        )
        new_pos = self.widget.integration_pattern_widget.pattern_view.pos_line.getPos()[
            0
        ]
    ...
    def test_pos_line_updates_on_integration_unit_change(self):
        ...
        self.assertEqual(
            new_pos,
            convert_units(
                10, self.model.calibration_model.wavelength, "2th_deg", "q_A^-1"
            ),
        )
        ...
        self.assertAlmostEqual(
            new_pos,
            convert_units(
                10, self.model.calibration_model.wavelength, "2th_deg", "d_A"
            ),
        )
    ...
    def test_pos_line_updates_on_tth_clicked_signal(self):
        ...
        self.assertAlmostEqual(
            new_pos,
            convert_units(
                3, self.model.calibration_model.wavelength, "2th_deg", "q_A^-1"
            ),
        )
