...
class IntegrationMockFunctionalTest(QtTest):
    ...
    def test_changing_number_of_integration_bins(self):
        ...
        enter_value_into_text_field(
            self.integration_widget.bin_count_txt, 2 * previous_number_of_points
        )
    ...
    def test_changing_supersampling_amount_integrating_to_cake_with_mask(self):
        ...
        self.assertEqual(
            self.model.calibration_model.pattern_geometry.pixel1, 0.5 * px1
        )
        self.assertEqual(
            self.model.calibration_model.pattern_geometry.pixel2, 0.5 * px2
        )
    ...
    def test_saving_image(self):
        ...
        QtWidgets.QFileDialog.getSaveFileName = MagicMock(
            return_value=os.path.join(data_path, "Test_img.png")
        )
        QTest.mouseClick(self.integration_widget.qa_save_img_btn, QtCore.Qt.LeftButton)
        QtWidgets.QFileDialog.getSaveFileName = MagicMock(
            return_value=os.path.join(data_path, "Test_img.tiff")
        )
        QTest.mouseClick(self.integration_widget.qa_save_img_btn, QtCore.Qt.LeftButton)
