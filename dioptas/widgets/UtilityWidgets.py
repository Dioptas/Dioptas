...
class MoveStageWidget(QtGui.QWidget):
    ...
    def check_sample_point_distances(self, posx, posy, posz):
        curx = float(epics.caget(epics_config['sample_position_x'], as_string=True))
        cury = float(epics.caget(epics_config['sample_position_y'], as_string=True))
        curz = float(epics.caget(epics_config['sample_position_z'], as_string=True))
