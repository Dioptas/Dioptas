...
class PhaseInScanController(object):
    """
    PhaseInScanController handles all the interaction between the phase controls and the plotted lines in the cake view.
    """

    def __init__(self, scan_widget, dioptas_model):
        """
        :param scan_widget: Reference to an IntegrationWidget
        :param dioptas_model: reference to DioptasModel object

        :type scan_widget: IntegrationWidget
        :type dioptas_model: DioptasModel
        """
        self.model = dioptas_model
        self.phase_model = self.model.phase_model
        self.scan_widget = scan_widget
        self.scan_view_widget = scan_widget.img_view  # type: IntegrationImgWidget

        self.connect()

    def connect(self):
        self.phase_model.phase_added.connect(self.add_phase_plot)
        self.model.phase_model.phase_removed.connect(self.scan_view_widget.del_cake_phase)
    ...
    def add_phase_plot(self):
        ...
        self.scan_view_widget.add_cake_phase(cake_line_positions, cake_line_intensities,
    ...
    def update_phase_lines(self, ind):
        ...
        self.scan_view_widget.update_phase_intensities(ind, cake_line_positions, cake_line_intensities)

    def update_phase_color(self, ind):
        self.scan_view_widget.set_cake_phase_color(ind, self.model.phase_model.phase_colors[ind])

    def update_phase_visible(self, ind):
        if self.phase_model.phase_visible[ind] and self.scan_widget.phases_btn.isChecked():
            self.scan_view_widget.show_cake_phase(ind)
        else:
            self.scan_view_widget.hide_cake_phase(ind)

    def reflection_added(self, ind):
        self.scan_view_widget.phases[ind].add_line()

    def reflection_deleted(self, phase_ind, reflection_ind):
        self.scan_view_widget.phases[phase_ind].delete_line(reflection_ind)
