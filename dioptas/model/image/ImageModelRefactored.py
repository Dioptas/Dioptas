...
from .ImageCommands import ImageCommandProcessor, BackgroundDimensionWrongException, RotationDirection, FlipDirection
...
class ImageModelRefactored:
    ...
    def _update_state(self, new_state: ImageState):
        ...
        if (old_state.raw_image_data is not new_state.raw_image_data or
            old_state.background_data is not new_state.background_data):
    ...
    def set_factor(self, factor: float) -> ImageState:
        ...
        new_state = self._command_processor.set_factor(
    ...
    def _apply_transformations(self, state: ImageState) -> ImageState:
        ...
        transformed_background = state.background_data
        if transformed_background is not None:
            transformed_background = self._command_processor.transformer._perform_background_transformations(
